...
const InflowProfile: React.FC<InflowProfileProps> = ({ state, updateState }) => {
  ...
  const IncomeInput = ({ label, icon: Icon, value, field }: any) => (
    <div className="group bg-white p-8 rounded-[2.5rem] border border-slate-200 hover:border-indigo-400 transition-all shadow-sm">
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-24">
      ...
      <div className="bg-[#0b0f1a] p-12 md:p-16 rounded-[4rem] text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-12">
          <div className="space-y-6">
            ...
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter leading-[0.85]">Inflow <br/><span className="text-indigo-500">Analysis.</span></h2>
            <p className="text-slate-400 text-lg font-medium max-w-lg leading-relaxed">
              Consolidated mapping of all household income channels. Adjust <span className="text-white">Active Salary</span> and <span className="text-white">Passive Yields</span>.
            </p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 p-10 rounded-[4rem] backdrop-blur-xl flex flex-col items-center gap-3 shadow-inner min-w-[320px]">
             <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Total Monthly Inflow</p>
             <h4 className="text-4xl md:text-5xl font-black tracking-tighter text-emerald-400">
      ...
      <div className="space-y-6">
        <h3 className="px-2 text-[11px] font-black text-slate-400 uppercase tracking-widest">Household Earner Node</h3>
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
          {members.map(member => (
            <button
              key={member.id}
              onClick={() => setSelectedId(member.id)}
              className={`relative flex-shrink-0 w-56 p-7 rounded-[2.5rem] border-2 transition-all text-left group ${
                selectedId === member.id 
                  ? 'bg-slate-900 border-slate-900 text-white shadow-2xl' 
                  : 'bg-white border-slate-200 text-slate-900 hover:border-indigo-300'
              }`}
            >
              <p className={`text-[10px] font-black uppercase mb-1 ${selectedId === member.id ? 'text-slate-500' : 'text-slate-400'}`}>{member.relation}</p>
              ...
              <div className="mt-6 flex items-center justify-between">
                ...
                {selectedId === member.id && <CheckCircle2 size={16} className="text-indigo-400" />}
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-8">
           <h4 className="px-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Core Professional Earnings</h4>
        ...
        <div className="space-y-8">
           <h4 className="px-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Secondary & Passive Flows</h4>
