...
const ActionPlan: React.FC<{ state: FinanceState }> = ({ state }) => {
  const calculations = useMemo(() => {
    ...
    const deficit = yearlyInflow - totalOutflowPa;
  ...
  return (
    <div className="space-y-8 md:space-y-12 animate-in fade-in duration-1000 pb-24">
      ...
      <div className="bg-[#0b0f1a] p-8 md:p-16 rounded-[2.5rem] md:rounded-[5rem] text-white relative overflow-hidden shadow-2xl shadow-indigo-900/30">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 md:gap-12">
          <div className="space-y-4 md:space-y-6">
            ...
            <h2 className="text-3xl md:text-7xl font-black tracking-tighter leading-tight md:leading-[0.85]">Strategic <br/><span className="text-indigo-500">Actions.</span></h2>
          ...
          <div className="flex flex-col gap-4 w-full md:w-auto">
            <div className="bg-white/5 border border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[4rem] backdrop-blur-xl flex flex-col items-center gap-2 md:gap-3 shadow-inner md:min-w-[320px]">
               <p className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest">Strategy Health Score</p>
               <h4 className="text-4xl md:text-5xl font-black text-white tracking-tighter">82%</h4>
               <div className="flex items-center gap-2 mt-1 md:mt-2 text-[9px] md:text-[10px] font-bold text-amber-400 uppercase tracking-widest bg-amber-500/10 px-3 md:px-4 py-1 md:py-1.5 rounded-full border border-amber-500/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {actions.map((action) => (
          <div key={action.id} className="bg-white p-8 md:p-10 rounded-[2rem] md:rounded-[4rem] border border-slate-200 shadow-sm relative overflow-hidden group hover:border-indigo-400 transition-all flex flex-col h-full">
            <div className="flex justify-between items-start mb-6 md:mb-8">
              <div className={`p-4 md:p-5 rounded-2xl md:rounded-3xl bg-${action.color}-50 text-${action.color}-600 group-hover:bg-${action.color}-600 group-hover:text-white transition-all`}>
                <action.icon size={24} md:size={28} />
              </div>
              <span className={`px-3 md:px-4 py-1.5 md:py-2 bg-${action.color}-50 text-${action.color}-600 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest border border-${action.color}-100`}>
                {action.priority}
              </span>
            </div>
            
            <div className="flex-1 space-y-4 md:space-y-6">
              <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-tight">{action.title}</h3>
              <p className="text-xs md:text-sm font-medium text-slate-500 leading-relaxed">{action.description}</p>
              
              <div className="space-y-2 md:space-y-3 pt-2 md:pt-4">
                {action.tactics.map((tactic, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100 group/item hover:border-indigo-200 transition-all">
                    <CheckCircle2 size={14} md:size={16} className="text-slate-300 mt-0.5 group-hover/item:text-indigo-600 transition-colors shrink-0" />
                    <span className="text-[11px] md:text-xs font-bold text-slate-600 group-hover/item:text-slate-900 transition-colors">{tactic}</span>
                  </div>
                ))}
              </div>
            </div>

            <button className="mt-8 md:mt-10 w-full py-4 md:py-5 bg-slate-900 text-white rounded-[1.5rem] md:rounded-[2rem] font-black text-[9px] md:text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center justify-center gap-2">
      ...
      <div className="bg-slate-950 p-8 md:p-12 rounded-[2.5rem] md:rounded-[5rem] text-white flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12 relative overflow-hidden shadow-2xl">
         <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 blur-[80px] rounded-full" />
         <div className="space-y-3 md:space-y-4 relative z-10 text-center md:text-left">
            <h4 className="text-2xl md:text-3xl font-black tracking-tight flex items-center justify-center md:justify-start gap-3">
              <Sparkles size={20} md:size={24} className="text-indigo-400" /> Efficiency Audit
            </h4>
         ...
         <div className="flex gap-4 md:gap-6 relative z-10">
            <div className="text-center">
              <p className="text-[8px] md:text-[10px] font-black text-slate-500 uppercase mb-1 md:mb-2">Build Rate</p>
              <p className="text-2xl md:text-3xl font-black text-emerald-400">{calculations.investmentRatio.toFixed(1)}%</p>
            </div>
            <div className="w-[1px] h-10 md:h-12 bg-white/10" />
            <div className="text-center">
              <p className="text-[8px] md:text-[10px] font-black text-slate-500 uppercase mb-1 md:mb-2">Burn Rate</p>
              <p className="text-2xl md:text-3xl font-black text-rose-400">{(100 - calculations.investmentRatio).toFixed(1)}%</p>
            </div>
         </div>
      </div>
    </div>
